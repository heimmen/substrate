...
#
# TEST environment for the mf-pi demo: deploys into the ate-demo-mf-pi-test
# namespace using the mfpi-test atespace (see demos/mf-pi/mf-pi-test.yaml.tmpl),
# so it never conflicts with the production demo-mf-pi.
# Mirror of demos/mf-pi/deploy-test.sh; keep the two in sync.

ATE_DEMOS+=(demo-mf-pi-test) # register demo-mf-pi-test

demo-mf-pi-test_cmdline() {
  case "${1}" in
    --deploy-demo-mf-pi-test) demo-mf-pi-test_deploy ;;
    --delete-demo-mf-pi-test) demo-mf-pi-test_delete ;;
    *)
      return 1
      ;;
  esac
...
# The workload images are shared with the production demo. A MinIO image
# missing from the repo is auto-localized from the local docker cache.
demo-mf-pi-test_images() {
...
demo-mf-pi-test_sync_secrets() {
  ...
  if [[ -z "${MINIO_ROOT_PASSWORD:-}" ]]; then
    MINIO_ROOT_PASSWORD="$(run_kubectl get secret mfpi-minio-admin -n ate-demo-mf-pi-test \
...
# Render mf-pi-test.yaml.tmpl. Unset values fall back to placeholders so the
# output is always valid YAML (the delete path identifies resources by metadata
# alone).
demo-mf-pi-test_render() {
...
demo-mf-pi-test_deploy() {
  log_step "demo-mf-pi-test_deploy"
  ...
  if ! demo-mf-pi-test_images; then
    return 1
  fi
  ...
  MFPI_WORKER_REPLICAS="${MFPI_WORKER_REPLICAS:-2}"
  ...
  local manifest
  manifest="$(mktemp)"
  demo-mf-pi-test_render > "${manifest}"
  ensure_at_recreate_if_changed "${manifest}" ate-demo-mf-pi-test mf-pi
  ...
  run_kubectl rollout status deployment/mfpi-admin -n ate-demo-mf-pi-test --timeout=120s
}

demo-mf-pi-test_delete() {
  log_step "demo-mf-pi-test_delete"
  delete_demo_actors ate-demo-mf-pi-test mf-pi
  demo-mf-pi-test_render | run_kubectl delete --ignore-not-found -f -
}
