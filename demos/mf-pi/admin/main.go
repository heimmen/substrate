...
// mf-pi user-management server.
//
// Tiny stdlib HTTP server that proxies to the substrate ateapi gRPC service
// and exposes a small JSON API + embedded web UI for managing mf-pi users
// (each user is one Actor in the mfpi atespace):
//
...
//	GET    /_mfpi_auth                nginx auth_request target: validate the
...
const (
	defaultPort              = "8080"
	defaultAtespace          = "mfpi"
	defaultTemplateNamespace = "ate-demo-mf-pi"
	defaultTemplateName      = "mf-pi"
...
// mfpi_user cookie rather than by a username in the URL. Unlike mf-cc, the
// pi-web frontend issues every request (API, plugin assets, WebSockets)
// relative to the /<username>/ base, so the only reserved prefix is the
// user-management UI itself; the bare-origin fallback (favicon etc.) is
// routed by the cookie. Keep in sync with nginx.conf.
var reservedPrefixRe = regexp.MustCompile(`^/(usermanagement)(/|$)`)
...
// the original request URI (user-scoped path) or the mfpi_user cookie
// (bare-origin fallback), then requires HTTP Basic credentials where the
// username matches the target user and the password matches the stored hash.
...
// paths (/alice/...) it is the leading path segment — which covers every
// pi-web request, since the frontend resolves API/plugin/WebSocket URLs
// against the /<username>/ base. For the bare-origin fallback (favicon etc.)
// it is the mfpi_user cookie.
func targetUser(r *http.Request) string {
	...
	return r.Header.Get("X-Mfpi-User")
}

func unauthorized(w http.ResponseWriter, msg string) {
	w.Header().Set("WWW-Authenticate", `Basic realm="mfpi agent", charset="UTF-8"`)
