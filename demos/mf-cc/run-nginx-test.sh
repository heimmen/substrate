...
# then run the mfcc-nginx-test container. Port-forwards already in use are
...
# local ports (59880/59882), the ate-demo-mf-cc-test namespace, a distinct
# container name (mfcc-nginx-test), a distinct htpasswd path and bind-mounts
# nginx-test.conf over the image's baked-in conf. The production environment
# (58880/58882, mfcc-nginx, nginx.conf) is left untouched.
#
#   59880 -> svc/atenet-router:80     (actor traffic)
#   59882 -> svc/mfcc-admin:8080      (user-management UI, test ns)
...
port_forward 59882 ate-demo-mf-cc-test mfcc-admin 8080
...
HTPASSWD_FILE="${TMPDIR:-/tmp}/mfcc-admin-test.htpasswd"
printf '%s:%s\n' "$ADMIN_USER" "$(openssl passwd -apr1 "$ADMIN_PASSWORD")" > "$HTPASSWD_FILE"
chmod 644 "$HTPASSWD_FILE"

docker rm -f mfcc-nginx-test 2>/dev/null || true
docker run -d -p 59881:59881 --name mfcc-nginx-test --network host \
  -v "$HTPASSWD_FILE:/etc/nginx/mfcc-admin.htpasswd:ro" \
  -v "$PWD/nginx-test.conf:/etc/nginx/conf.d/default.conf:ro" \
  mfcc-nginx

echo "mfcc-nginx-test running."
