...
#
# Deploy the mf-cc demo (mf-cc actors + the mfcc-admin user-management UI)
...
#   MFCC_WORKER_REPLICAS  (default 16)
...
render() {
  sed -e "s|\${BUCKET_NAME}|$(esc_repl "${BUCKET_NAME:-placeholder}")|g" \
      ...
      -e "s|\${MFCC_WORKER_REPLICAS}|$(esc_repl "${MFCC_WORKER_REPLICAS:-16}")|g" \
      "${TEMPLATE}"
}
...
cmd_deploy() {
  ...
  echo "  worker replicas:       ${MFCC_WORKER_REPLICAS:-16}"
  ...
  echo "mf-cc demo deployed."
  echo "  management UI: run ./run-nginx.sh, then open http://localhost:58881/usermanagement/"
}
